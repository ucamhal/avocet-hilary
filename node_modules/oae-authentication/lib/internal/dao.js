...
var getUserIdsFromLoginIds = module.exports.getUserIdsFromLoginIds = function(loginIds, callback) {
    ...
    Cassandra.runQuery('SELECT "loginId", "userId" FROM "AuthenticationLoginId" WHERE "loginId" IN (?)', [flatLoginIds], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var getUserLoginIds = module.exports.getUserLoginIds = function(userId, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "AuthenticationUserLoginId" WHERE "userId" = ?', [userId], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
var getUsersLoginIds = module.exports.getUsersLoginIds = function(userIds, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "AuthenticationUserLoginId" WHERE "userId" IN (?)', [userIds], function(err, rows) {
        if (err) {
            return callback(err);
        }
        ...
        _.each(rows, function(row) {
            ...
            loginIds[userId] = loginIds[userId] || {};

            if (row.get('loginId') && row.get('loginId').value) {
                var loginId = _expandLoginId(row.get('loginId').value);
                if (loginId) {
...
var changePassword = module.exports.changePassword = function(loginId, newPassword, callback) {
    Cassandra.runQuery('UPDATE "AuthenticationLoginId" SET "password" = ? WHERE "loginId" = ?', [newPassword, _flattenLoginId(loginId)], callback);
};
