...
/**
 * Create a new user record on the current tenant.
 * Note that you will still need to associate an authentication strategy with this user record
 * as the user would otherwise have no means of logging onto the system.
 *
...
var createUser = module.exports.createUser = function(ctx, displayName, opts, callback) {
    ...
    opts.locale = opts.locale || PrincipalsConfig.getValue(tenantAlias, 'user', 'defaultLanguage');
 ...
 * @param  {Boolean}        [forceProfileUpdate]    Whether or not the user's display name, public alias and should be updated with the value specified in the CSV file, even when the display name/public alais is different than the external id or an email address has been set. By default, this will be set to `false`
...
var importUsers = module.exports.importUsers = function(ctx, tenantAlias, userCSV, authenticationStrategy, forceProfileUpdate, callback) {
    ...
    // If no `forceProfileUpdate` flag has been provided, we default to `false`
    ...
    parser.on('finish', function() {
        ...
        _cleanUpCSVFile(userCSV, function() {
            ...
            var processUser = function(data) {
                ...
                require('oae-authentication').getOrCreateUser(adminCtx, authenticationStrategy, externalId, displayName, opts, function(err, user, created) {
                    if (err) {
...
/**
 * Set a flag that indicates whether a user is a global admin. The ctx user must be a global admin to be able
 * to do this.
 *
 * @param  {Context}   ctx             Context that holds the tenant
