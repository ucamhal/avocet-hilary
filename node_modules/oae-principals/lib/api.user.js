 ...
 * @param  {String}     [tenantAlias]               The target tenant in which to create the user. If unspecified, will default to the tenant associated to the current request context
...
var createUser = module.exports.createUser = function(ctx, tenantAlias, displayName, opts, callback) {
    ...
    if (!opts.locale) {
        ...
        // If a user is creating a tenant on behalf of someone else or there was no suggested
...
var updateUser = module.exports.updateUser = function(ctx, userId, profileFields, callback) {
    ...
    validator.check(userId, {'code': 400, 'msg': 'A valid user id must be provided'}).isPrincipalId();
 ...
 *
 * @param  {Context}   ctx              Context that holds the tenant
 * @param  {User}      userId           The id of the user whose admin status to set or unset
...
var setGlobalAdmin = module.exports.setGlobalAdmin = function(ctx, principalId, isAdmin, callback) {
    if (ctx.user() && _.isFunction(ctx.user().isGlobalAdmin) && ctx.user().isGlobalAdmin()) {
        return _setAdmin(ctx, 'admin:global', isAdmin, principalId, callback);
    } else {
        return callback({'code': 401, 'msg': 'You do not have sufficient rights to make someone an admin.'});
    }
};
