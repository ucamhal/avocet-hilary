...
var getAuthzMembers = module.exports.getAuthzMembers = function(resourceId, start, limit, callback) {
    ...
    Cassandra.runPagedColumnQuery('AuthzMembers', 'resourceId', resourceId, start, limit, null, function(err, columns, nextToken) {
        if (err) {
            return callback(err);
        }

        var members = _.map(columns, function(column) {
            return {'id': column.name, 'role': column.value};
        });
...
var getPrincipalMemberships = module.exports.getPrincipalMemberships = function(principalId, start, limit, callback) {
    ...
    Cassandra.runPagedColumnQuery('AuthzMembershipsCache', 'principalId', principalId, start, limit, null, function(err, columns, nextToken, startMatched) {
        if (err) {
            return callback(err);
        }

        if (columns.length > 0 || startMatched) {
            // If we retrieved columns or the start matched a column exactly, then we know we had a cache hit
            return callback(null, _.pluck(columns, 'name'), nextToken);
        } else {
...
var getRolesForPrincipalAndResourceType = module.exports.getRolesForPrincipalAndResourceType = function(principalId, resourceType, start, limit, callback) {
    limit = OaeUtil.getNumberParam(limit, 1, 10);
    ...
    Cassandra.runQuery(cql, [start, end, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
...
/**
 * Checks to see if both the user and the resource's tenant can interact with the given list of principals. This will **not**
 * perform any interaction or permissions checks between the user in context and resource, that must be performed separately.
 *
 * @param  {Context}        ctx                             The context of the current request
 * @param  {String}         resourceTenantAlias             The tenant alias of the resource the user is performaning an action upon.
 * @param  {Principal[]}    principals                      The principals with which the user wishes to interact with
 * @param  {Function}       callback                        Invoked when the process completes
 * @param  {Object}         callback.err                    An error that occurred, if any
 ...
 */
var canInteract = module.exports.canInteract = function(ctx, resourceTenantAlias, principals, callback) {
    ...
    // If there are any conflicts with the resource, return an error immedliately
    ...
    var checkPrincipal = function(principals) {
        ...
        if (AuthzUtil.isUserId(principal.id)) {
            resolveImplicitRole(ctx, principal.id, principal.tenant.alias, principal.visibility, ['viewer', 'manager'], function(err, implicitRole, canInteractWithUser) {
                if (err) {
                    return callback(err);
                }
