...
describe('Tickets - ZenDesk', function() {
    ...
    describe('Creating tickets', function() {
        ...
        it('verify that no ZenDesk tickets are created when disabled for tenant', function(callback) {
            ...
            TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, camTenant, {'ticketId': 't:camtest:d2c7fb55lK', 'externalId': 'OA-1'}, function(err, ticket) {
        ...
        it('verify parameters', function(callback) {
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets-zendesk/general/forwardToZendesk': true}, function(err) {
                ...
                TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, null, {}, function(err, ticket) {
                    assert.equal(err.code, 400);

                    // Verify that creating a ZenDesk ticket with a missing ticket throws an error
                    TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, camTenant, null, function(err, ticket) {
                        assert.equal(err.code, 400);

                        // Verify that creating a ZenDesk ticket with an invalid ticket throws an error
                        TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, camTenant, 'invalid_entry', function(err, ticket) {
                            assert.equal(err.code, 400);

                            // Verify that creating a ZenDesk ticket with an invalid ticket throws an error
                            TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, camTenant, [], function(err, ticket) {
                                assert.equal(err.code, 400);
        ...
        it('verify that creating a ticket generates a ZenDesk ticket', function(callback) {
            ...
            RestAPI.Publications.createPublication(coenegoUser.restContext, ['foobar#' + Date.now()], 'A displayName', 'other', 1388676959000, [coenegoUser.user.displayName], null, null, function(err, data, result) {
                assert.ok(!err);
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets-zendesk/general/forwardToZendesk': true}, function(err) {
                    ...
                    // Verify that creating a ticket when the integration is disabled results in an error
                    TicketsZenDeskAPI.createZenDeskTicket(coenegoUser, camTenant, data.publication.ticket, function(err, ticket) {
                        assert.ok(!err);
