...
var TicketsRestAPI = require('oae-tickets/lib/test/util');
...
describe('Tickets - ZenDesk', function() {
    ...
    describe('Creating tickets', function() {
        ...
        it('verify that creating a ticket generates a ZenDesk ticket', function(callback) {
            ...
            // Mock endpoint that updates a ticket
            ...
            RestAPI.Content.createFile(camAdminRestContext, 'Some file', null, 'public', getFileStream, null, null, function(err, content) {
                ...
                // Create a new ticket
                TicketsRestAPI.createTicket(camAdminRestContext, {'contentId': content.id, 'sourceIds': ['foobar#' + Date.now()], 'publication': generatePublicationData}, function(err, ticket) {
                    assert.ok(!err);
                    ...
                    ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets-zendesk/general/forwardToZendesk': true}, function(err) {
                        ...
                        // Create a ZenDesk ticket, specifying the previously created ticket and publication
                        TicketsZenDeskAPI.createZenDeskTicket(camUserCtx, coenegoUser, camTenant, ticket, function(err, zenDeskTicket) {
                            assert.ok(!err);
                            assert.ok(zenDeskTicket);
                            assert.strictEqual(ticketsCreated, 1);
                            assert.strictEqual(zenDeskTicket.id, ticketsCreated);
                            assert.strictEqual(zenDeskTicket.external_id, ticket.externalId);
                            assert.strictEqual(zenDeskTicket.created_at, '2014-04-14T17:13:05Z');
                            assert.strictEqual(zenDeskTicket.updated_at, '2014-04-14T17:13:05Z');
                            assert.strictEqual(zenDeskTicket.subject, util.format('Open Access enquiry %s', ticket.externalId));
                            assert.strictEqual(zenDeskTicket.status, 'new');
                            assert.strictEqual(zenDeskTicket.requester_id, 479860142);
                            assert.strictEqual(zenDeskTicket.submitter_id, 479860142);
                            assert.strictEqual(zenDeskTicket.assignee_id, 477098892);
                            assert.strictEqual(zenDeskTicket.group_id, '123456');
                            return callback();
                        });
                    });
                });
