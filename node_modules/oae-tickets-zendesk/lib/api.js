    ...
    PrincipalsDAO.getPrincipal(ticket.createdBy, function(err, user) {
        ...
        PublicationsAPI.getPublication(ctx, ticket.publicationId, function(err, publication) {
            ...
            _getOrCreateZendeskUser(client, user.displayName, userEmail, function(err, zenDeskUser) {
                ...
                // Construct the URL for the publication
                var publicationIdWithoutTenant = /^p:[^:]+:([^:]+)$/.exec(publication.id)[1];
                var publicationURL = util.format('https://%s/publications/%s/%s', tenant.host, tenant.alias, publicationIdWithoutTenant);
                ...
                client.tickets.create(zenDeskTicket, function(err, req, result) {
                    ...
                    var zenDeskComment = {
                        'ticket': {
                            'comment': {
                                'body': ZenDeskTemplates.zendeskTicketCommentBodyTemplate({
                                    'user': user,
                    ...
                    client.tickets.update(result.id, zenDeskComment, function(err) {
                        if (err) {
                            log().error({'err': err, 'zenDeskComment': zenDeskComment}, 'Error commenting on ticket');
                            return callback({'code': 500, 'msg': 'Error commenting on the ZenDesk ticket'});
                        }
