...
var Zendesk = exports.Zendesk = function Zendesk(options) {
    ...
    var zendesk = options.nodeZendesk || NodeZendesk;

    that.zendeskClientFromOAEConfig = function(ctx) {
        var tenantAlias = ctx.user().tenant.alias;

        var username = config.getZendeskEmail(tenantAlias);
        var token = config.getZendeskApiToken(tenantAlias);
        var zendeskUrl = config.getZendeskUrl(tenantAlias);
