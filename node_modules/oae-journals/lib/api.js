...
var importJournals = module.exports.importJournals = function(ctx, journalsCSV, callback) {
    ...
    csv().from.path(journalsCSV.path).to.array(function(journalsData) {
        ...
        _.each(journalsCollection, function(journal) {
            validator.check(journal.id, {'code': 400, 'msg': 'An invalid ID was passed in for journal: ' + journal.displayName}).notEmpty();
            validator.check(journal.displayName, {'code': 400, 'msg': 'An invalid displayName was passed in for journal: ' + journal.id}).notEmpty();
        });
        if (validator.hasErrors()) {
            log().error(validator.getFirstError(), 'Error while importing journals from .CSV file');
            return cleanCallback(validator.getFirstError());
