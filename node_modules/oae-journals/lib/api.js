    ...
    csv().from.path(journalsCSV.path).to.array(function(journalsData) {
        ...
        _.each(journalsData, function(journal) {
            ...
            if (journal[0] && (_isValidISSN(journal[1]) || _isValidISSN(journal[2]))) {
                ...
                log().error({'code': 400, 'msg': util.format('Journal "%s" not imported because of invalid input', journal[0])}, 'Error while indexing journal');
            }
        });
...
var _concatJournalIds = function(printISSN, electronicISSN) {
    ...
    var cleanUpISSN = function(ISSN) {
        if (ISSN) {
            ...
            ISSN = [];
            for (var i in Ids) {
                ISSN.push(Ids[i].trim());
            }
            return(_.compact(ISSN).join('-'));
        }
        return null;
    };
