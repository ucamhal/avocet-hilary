...
describe('Journals', function() {
    ...
    /*!
     * Disable importing journals before each test
     */
    beforeEach(function(callback) {
        ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-journals/general/enabled': false}, function(err) {
            assert.ok(!err);
            callback();
        });
    });
    ...
    describe('#importJournals()', function() {
        ...
        it('verify parameters', function(callback) {
            RestAPI.Journals.importJournals(globalAdminRestContext, null, function(err) {
                assert.equal(err.code, 400);

                RestAPI.Journals.importJournals(globalAdminRestContext, [], function(err) {
                    assert.equal(err.code, 400);
            ...
            RestAPI.Journals.importJournals(anonymousGlobalRestContext, {'journals': getFileStream()}, function(err) {
                ...
                RestAPI.Journals.importJournals(anonymousCamRestContext, {'journals': getFileStream()}, function(err) {
                    ...
                    RestAPI.Journals.importJournals(camUserRestContext, {'journals': getFileStream()}, function(err) {
                        ...
                        RestAPI.Journals.importJournals(camAdminRestContext, {'journals': getFileStream()}, function(err) {
                            assert.equal(err.code, 401);

                            RestAPI.Journals.importJournals(globalAdminRestContext, {'journals': getFileStream()}, function(err) {
                                assert.ok(!err);

                                // Enable importing journals for tenant
                                ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-journals/general/enabled': true}, function(err) {
                                    assert.ok(!err);
                ...
                RestAPI.Journals.importJournals(camAdminRestContext, {'journals': getFileStream()}, function(err) {
                    ...
                    SearchTestsUtil.searchRefreshed(camUserRestContext, 'journals', null, {'q': 'Academic'}, function(err, data) {
                        ...
                        _.each(data.results, function(journal) {
                            ...
                            assert.notEqual(_.indexOf(journal.displayName, 'Academic'), -1);
                        });
                        return callback();
                    });
                });
