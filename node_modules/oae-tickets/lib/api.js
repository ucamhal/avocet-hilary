...
var _canManage = function(ctx, publication) {
    ...
    // Global admins should always be allowed to create tickets
    if (ctx.user().isGlobalAdmin()) {
        return true;
    }

    // Check if creating tickets is enabled for the other users
    var enabled = TicketsConfig.getValue(ctx.tenant().alias, 'general', 'enabled');
    if (!enabled) {
        return false;
    }

    // Tenant admins are allowed to create tickets when module is enabled
    if (ctx.user().isAdmin(publication.tenantAlias)) {
        return true;
    }
