...
var createTicket = module.exports.createTicket = function(ctx, contentId, sourceIds, publication, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        if (publication) {
            ...
            // Convert the value of `underEmbargo` to a boolean
            publication.underEmbargo = (publication.underEmbargo === 'true');
            ...
            if (publication.underEmbargo) {
                validator.check(publication.embargoDate, {'code': 400, 'msg': 'Missing embargoDate'}).notEmpty();
                if (publication.embargoDate) {
                    validator.check(publication.embargoDate, {'code': 400, 'msg': 'Invalid embargoDate'}).isNumeric();
                }
            } else {
                if (publication.embargoDate) {
                    publication.embargoDate = 0;
                }
            }
        }
