...
var getTickets = module.exports.getTickets = function(ctx, ticketIds, callback) {
    ...
    return TicketsDAO.getTickets(ticketIds, function(err, tickets) {
        ...
        // Check whether the user has access to the requested tickets
        var noAccess = _.some(tickets, function(ticket) {
            return !_canRequestTicket(ctx, ticket);
        });

        return noAccess ? callback({'code': 401, 'msg': 'Only authorized users are allowed to request tickets'}) : callback(null, tickets);
    });
};
