...
PublicationsAPI.registerPublicationsDecorator('ticket', function(ctx, publications, callback) {
    return _addTicketsToPublications(publications, callback);
});
        ...
        var _createTicket = function() {
            ...
            Locking.acquire(key, 15, function(err, lockToken) {
                ...
                if (err) {
                    ...
                    return callback({'code': 400, 'msg': util.format('Unable to create ticket for publication: "%s"', publicationId)});
                }

                // Increase the current counter value
                TicketsDAO.increaseTicketsCounter(function(err, counterValue) {
                    if (err) {
                        log().error({'code': err.code, 'msg': err.msg},'Error while creating ticket ID');
                    ...
                    TicketsDAO.createTicket(ticketId, parameters, function(err, ticket) {
                        if (err) {
                            log().error({'code': err.code, 'msg': err.msg, 'ticketId': ticketId, 'parameters': parameters}, 'Error while creating a ticket');
                            return _releaseLocking(lockToken, err);
                        }
 ...
 * @api private
 */
var _addTicketsToPublications = function(publications, callback) {
...
var _canEnterData = function(ctx) {
    return (ctx.user() && TicketsConfig.getValue(ctx.tenant().alias, 'general', 'enabled'));
};

/**
 * Verifies that the current user is allowed to edit the ticket
 *
 * @param  {Object}     ctx                 The current context
