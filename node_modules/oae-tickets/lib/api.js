...
var createTicket = module.exports.createTicket = function(ctx, contentId, sourceIds, publication, callback) {
    ...
    ContentDAO.Content.getContent(contentId, function(err, content) {
        ...
        if (publication) {
            ...
            validator.check(publication.acceptanceDate, {'code': 400, 'msg': 'Missing or invalid acceptanceDate'}).isNumeric();
            validator.check(publication.funders, {'code': 400, 'msg': 'Missing funders'}).notEmpty();
            validator.check(null, {'code': 400, 'msg': 'Missing funders'}).isArray(publication.funders);
...
var getTickets = module.exports.getTickets = function(ctx, ticketIds, callback) {
    ...
    return TicketsDAO.getTickets(ticketIds, function(err, tickets) {
        ...
        // TODO: heck if the user is allowed to request the tickets
 ...
 * @api private
 */
var _canEnterData = function(ctx, content) {
    if (!ctx.user()) {
        return false;
    }
...
/**
 * Verifies that the current user is allowed to manage a ticket
 *
 * @param  {Object}     ctx         The current context
 ...
 * @api private
 */
var _canManage = function(ctx, ticket) {
    if (!ctx.user()) {
        return false;
    }
