...
describe('Tickets', function() {
    ...
    var isCorrespondingPublication = function(publication, ticket) {
        assert.ok(_.isObject(publication));

        // Find the list of keys which have undefined values on the publication
        var undefinedKeys = _.chain(publication).map(function(value, key) {
            return value === undefined ? key : null;
        }).compact().value();

        // Define the fields which are auto generated (so won't match in the ticket's publication)
        var generatedFields = _.flatten([['id', 'createdBy', 'linkedContentId'], undefinedKeys], true);

        // Ensure that the generated fields are all falsy values in our publication
        _.each(generatedFields, function(field) {
            assert.ok(!publication[field], util.inspect(publication[field]));
        });

        // Ensure our publication matches the ticket's publication, except for the generated values
        var a = _.omit(publication, generatedFields);
        var b = _.omit(ticket.publication, generatedFields);
        assert.ok(_.isEqual(a, b), 'publications were not equal: ' + util.inspect([a, b]));
    };
            ...
            it('verify that requesting a publication comes with its corresponding ticket', function(callback) {
                ...
                RestAPI.Content.createFile(coenegoUser.restContext, 'Some file', null, 'public', getFileStream, null, null, function(err, content) {
                    ...
                    TicketsRestAPI.createTicket(coenegoUser.restContext, {'contentId': content.id, 'sourceIds': generateRandomSourceId, 'publication': generatePublicationData}, function(err, ticket) {
                        ...
                        // Requesting the publication as an admin should come without a ticket
            ...
            it('verify that requesting a publication comes with its corresponding ticket for authorized users only', function(callback) {
                ...
                RestAPI.Content.createFile(coenegoUser.restContext, 'Some file', null, 'public', getFileStream, null, null, function(err, content) {
                    ...
                    TicketsRestAPI.createTicket(coenegoUser.restContext, {'contentId': content.id, 'sourceIds': generateRandomSourceId, 'publication': generatePublicationData}, function(err, ticket) {
                        ...
                        // Requesting the publication as an anonymous user should result in an error
                        PublicationsRestAPI.getPublication(anonymousRestContext, ticket.publication.id, function(err) {
                            assert.strictEqual(err.code, 401);

                            // Requesting the publication as an regular user, who didn't create the ticket, should result in an error
                            PublicationsRestAPI.getPublication(halUser.restContext, ticket.publication.id, function(err) {
                                assert.strictEqual(err.code, 401);

                                // Requesting the publication as a tenant admin from another tenant, should result in an error
                                PublicationsRestAPI.getPublication(gtAdminRestContext, ticket.publication.id, function(err) {
                                    assert.strictEqual(err.code, 401);
