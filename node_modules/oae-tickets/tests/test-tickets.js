...
describe('Tickets', function() {
    ...
    /**
     * Utility method that checkis if a
     *
    ...
    var isValidTicket = function(ticket) {
        ...
        assert.strictEqual(ticket.status, TicketsConstants.statusses.PENDING);
    };
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, coenegoUser, halUser) {
                        ...
                        // Create a new publication
                        RestAPI.Publications.createPublication(coenegoUser.restContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A'], null, null, function(err, data, result) {
                            assert.ok(!err);
                            assert.ok(data.publication);
                            createdPublications.push(data.publication);

                            // Create a new publication
                            RestAPI.Publications.createPublication(halUser.restContext, [generateRandomSourceId()], 'displayName B', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author B'], null, null, function(err, data, result) {
                                assert.ok(!err);
                                assert.ok(data.publication);
                                createdPublications.push(data.publication);

                                // Requesting the publications created by the Cam user should be decorated with their corresponding tickets when using the tenant admin
                                RestAPI.Publications.getPublicationsByCreator(camAdminRestContext, coenegoUser.user.id, null, null, function(err, data) {
                                    assert.ok(!err);
                                    assert.equal(data.results.length, 1);
                                    var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === data.results[0].id; });
                                    assert.ok(createdPublication);
                                    isValidTicket(data.results[0].ticket);
                                    isCorrespondingTicket(createdPublication, data.results[0].ticket);
