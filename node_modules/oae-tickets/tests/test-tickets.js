...
describe('Tickets', function() {
    ...
    before(function(callback) {
        ...
        TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, camUsers, _coenegoUser, _halUser) {
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                assert.ok(!err);
                return callback();
            });
        });
     ...
     * @param  {Error}                                      An assertion error is thrown if the `returnedTicket` is not associated with the `createdPublication`
     */
    var isCorrespondingTicket = function(createdPublication, returnedTicket) {
        ...
        assert.strictEqual(createdPublication.ticket.status, TicketsConstants.statusses.PENDING);
    };
     ...
     *
     * @param  {Ticket}         returnedTicket              Object representing a ticket that was requested
     * @param  {Error}                                      An assertion error is thrown if the tickets values are invalid
     */
    var isValidTicket = function(returnedTicket) {
        assert.ok(TicketsUtil.isTicketId(returnedTicket.ticketId));
        assert.ok(TicketsUtil.isExternalId(returnedTicket.externalId));
    };
        ...
        it('verify permissions', function(callback) {
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, camUser) {
                ...
                PublicationsDAO.createPublication(tenantAlias, publication, [generateRandomSourceId()], function(err, publication) {
                    ...
                    RestAPI.Tickets.createTicket(anonymousRestContext, {'publicationId': publication.id}, function(err, ticket) {
                        assert.equal(err.code, 401);
                        assert.ok(!ticket);

                        // Create a ticket with a registered user that doesn't have anything to do with the publication
                        RestAPI.Tickets.createTicket(camUser.restContext, {'publicationId': publication.id}, function(err, ticket) {
                            assert.equal(err.code, 401);
                            assert.ok(!ticket);

                            // Create a ticket with the creator of the publication
                            RestAPI.Tickets.createTicket(coenegoUser.restContext, {'publicationId': publication.id}, function(err, ticket) {
                                assert.equal(err.code, 401);
                                assert.ok(!ticket);

                                // Create a ticket with an author of the publication
                                RestAPI.Tickets.createTicket(halUser.restContext, {'publicationId': publication.id}, function(err, ticket) {
                                    assert.equal(err.code, 401);
                                    assert.ok(!ticket);

                                    // Create a ticket with the tenant admin
                                    RestAPI.Tickets.createTicket(gtAdminRestContext, {'publicationId': publication.id}, function(err, ticket) {
                                        assert.equal(err.code, 401);
                                        assert.ok(!ticket);

                                        // Create a ticket with the tenant admin
                                        RestAPI.Tickets.createTicket(camAdminRestContext, {'publicationId': publication.id}, function(err, ticket) {
                                            assert.equal(err.code, 401);
                                            assert.ok(!ticket);
                                            ...
                                            RestAPI.Admin.loginOnTenant(TestsUtil.createGlobalAdminRestContext(), 'localhost', function(err, ctx) {
                                                assert.ok(!err);
                                                RestAPI.Tickets.createTicket(ctx, {'publicationId': publication.id}, function(err, ticket) {
                                                    assert.ok(!err);
                                                    assert.ok(ticket);
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                ...
                TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenantAdminRestContext) {
                    ...
                    PublicationsDAO.createPublication(tenantAlias, publication, [generateRandomSourceId()], function(err, publication) {
                        ...
                        publication.ticket = {'status': TicketsConstants.statusses.PENDING};
                        ...
                        Locking.acquire(key, 15, function(err, lockToken) {
                            ...
                            RestAPI.Tickets.createTicket(tenantAdminRestContext, {'publicationId': publication.id}, function(err, ticket) {
                                assert.ok(err);
                                assert.equal(err.code, 400);
        ...
        it('verify creating tickets on different tenants doesn\'t cause conflicts with the external ID\'s', function(callback) {
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                ...
                TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenant1AdminRestContext) {
                    ...
                    RestAPI.Publications.createPublication(tenant1AdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data1, result) {
                        assert.ok(!err);
                        ...
                        TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenant2AdminRestContext) {
                            ...
                            RestAPI.Publications.createPublication(tenant2AdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data2, result) {
                                assert.ok(!err);
                                ...
                                RestAPI.Publications.createPublication(tenant1AdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data1, result) {
                                    ...
                                    RestAPI.Publications.createPublication(tenant2AdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data2, result) {
                                        ...
                                        TestsUtil.createTenantWithAdmin(tenantAlias, tenantAlias, function(err, tenant, tenant3AdminRestContext) {
                                            ...
                                            RestAPI.Publications.createPublication(tenant3AdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data, result) {
                                                assert.ok(!err);
                                                assert.ok(data.publication);
                            ...
                            TicketsAPI.getTicket({'ticketId': 't:camtest:0123456789'}, function(err, ticket) {
                                ...
                                TicketsAPI.getTicket('t:camtest:0123456789', function(err, ticket) {
                                    ...
                                    TicketsAPI.getTicket(createdPublication.ticket.ticketId, function(err, ticket) {
                                        ...
                                        assert.strictEqual(createdPublication.id, ticket.publicationId);
                                        assert.strictEqual(createdPublication.ticket.ticketId, ticket.ticketId);
                                        assert.strictEqual(createdPublication.ticket.externalId, ticket.externalId);
                                        assert.strictEqual(createdPublication.ticket.tenantAlias, ticket.tenantAlias);
                                        assert.strictEqual(createdPublication.ticket.publicationId, ticket.publicationId);
                                        assert.strictEqual(createdPublication.ticket.created, ticket.created);
                                        return callback();
                                    });
                                });
                            });
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                ...
                RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.displayName], null, null, function(err, data, result) {
                    ...
                    TicketsAPI.getTicket(data.publication.ticket.ticketId, function(err, ticket) {
                        ...
                        assert.ok(TicketsUtil.isTicketId(ticket.ticketId));
                        assert.ok(TicketsUtil.isExternalId(ticket.externalId));
                        assert.strictEqual(data.publication.id, ticket.publicationId);
                        assert.strictEqual(data.publication.ticket.ticketId, ticket.ticketId);
                        assert.strictEqual(data.publication.ticket.externalId, ticket.externalId);
                        assert.strictEqual(data.publication.ticket.tenantAlias, ticket.tenantAlias);
                        assert.strictEqual(data.publication.ticket.publicationId, ticket.publicationId);
                        assert.strictEqual(data.publication.ticket.created, ticket.created);
                        return callback();
                    });
                });
            });
            ...
            ConfigTestUtil.updateConfigAndWait(globalAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                ...
                RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.displayName], null, null, function(err, data, result) {
                    ...
                    TicketsDAO.getTicketsByPublicationIds([data.publication.id], function(err, tickets) {
                        ...
                        assert.ok(TicketsUtil.isTicketId(tickets[0].ticketId));
                        assert.ok(TicketsUtil.isExternalId(tickets[0].externalId));
    ...
    describe('#getPublicationsById()', function() {
        ...
        it('verify that a requested publication by its ID is only decorated with its corresponding ticket for authorized users', function(callback) {
            ...
            // Enable publications module for tenant
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                assert.ok(!err);
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, camUser) {
                        ...
                        // Create a new publication
                        RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [halUser.user.id], null, null, function(err, data, result) {
                            assert.ok(!err);
                            assert.ok(data.publication);
                            var createdPublication = data.publication;

                            // A requested publication should come WITHOUT a ticket when using the anonymous user
                            RestAPI.Publications.getPublication(anonymousRestContext, data.publication.id, function(err, publication) {
                                assert.ok(!err);
                                assert.ok(!publication.ticket);

                                // A requested publication should come WITHOUT a ticket when an admin from another tenant
                                RestAPI.Publications.getPublication(gtAdminRestContext, data.publication.id, function(err, publication) {
                                    assert.ok(!err);
                                    assert.ok(!publication.ticket);

                                    // A requested publication should come WITHOUT a ticket when using a user that hasn't anything to do with the publication
                                    RestAPI.Publications.getPublication(coenegoUser.restContext, data.publication.id, function(err, publication) {
                                        assert.ok(!err);
                                        assert.ok(!publication.ticket);

                                        // A requested publication should come WITH a ticket when using the creator
                                        RestAPI.Publications.getPublication(camUser.restContext, data.publication.id, function(err, publication) {
                                            assert.ok(!err);
                                            ...
                                            RestAPI.Publications.getPublication(halUser.restContext, data.publication.id, function(err, publication) {
                                                assert.ok(!err);
                                                isValidTicket(publication.ticket);
                                                isCorrespondingTicket(createdPublication, publication.ticket);
                                                ...
                                                RestAPI.Publications.getPublication(camAdminRestContext, data.publication.id, function(err, publication) {
                                                    ...
                                                    RestAPI.Admin.loginOnTenant(TestsUtil.createGlobalAdminRestContext(), 'localhost', function(err, ctx) {
                                                        assert.ok(!err);
                                                        RestAPI.Publications.getPublication(ctx, data.publication.id, function(err, publication) {
                                                            assert.ok(!err);
                                                            isValidTicket(publication.ticket);
                                                            isCorrespondingTicket(createdPublication, publication.ticket);
                                                            return callback();
        ...
        it('verify that a requested publication is decorated with its corresponding ticket', function(callback) {
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    // Create a new publication
                    RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'A displayName', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [coenegoUser.user.id], null, null, function(err, data, result) {
                        assert.ok(!err);
                        assert.ok(data.publication);
                        assert.ok(data.publication.ticket);
                        var createdPublication = data.publication;
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 1, function(err, users, camUser) {
                        ...
                        // Create a new publication
                        RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A'], null, null, function(err, data, result) {
                            assert.ok(!err);
                            assert.ok(data.publication);
                            createdPublications.push(data.publication);

                            // Create a new publication
                            RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName B', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [camUser.user.id], null, null, function(err, data, result) {
                                assert.ok(!err);
                                assert.ok(data.publication);
                                createdPublications.push(data.publication);

                                // Create a new publication
                                RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName C', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [camUser.user.id], null, null, function(err, data, result) {
                                    assert.ok(!err);
                                    assert.ok(data.publication);
                                    createdPublications.push(data.publication);
                                    ...
                                    RestAPI.Publications.getPublications(anonymousRestContext, camUser.user.id, null, null, function(err, data) {
                                        assert.ok(!err);
                                        ...
                                        RestAPI.Publications.getPublications(gtAdminRestContext, camUser.user.id, null, null, function(err, data) {
                                            assert.ok(!err);
                                            assert.equal(data.results.length, 2);
                                            _.each(data.results, function(publication) {
                                                assert.ok(!publication.ticket);
                                            ...
                                            RestAPI.Publications.getPublications(halUser.restContext, camUser.user.id, null, null, function(err, data) {
                                                assert.ok(!err);
                                                assert.equal(data.results.length, 2);
                                                _.each(data.results, function(publication) {
                                                    assert.ok(!publication.ticket);
                                                ...
                                                RestAPI.Publications.getPublications(camUser.restContext, camUser.user.id, null, null, function(err, data) {
                                                    assert.ok(!err);
                                                    assert.equal(data.results.length, 2);
                                                    _.each(data.results, function(publication) {
                                                    ...
                                                    RestAPI.Publications.getPublications(camAdminRestContext, camUser.user.id, null, null, function(err, data) {
                                                        assert.ok(!err);
                                                        assert.equal(data.results.length, 2);
                                                        _.each(data.results, function(publication) {
                                                            var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
                                                        ...
                                                        RestAPI.Admin.loginOnTenant(TestsUtil.createGlobalAdminRestContext(), 'localhost', function(err, ctx) {
                                                            assert.ok(!err);
                                                            RestAPI.Publications.getPublications(ctx, camUser.user.id, null, null, function(err, data) {
                                                                assert.ok(!err);
                                                                assert.equal(data.results.length, 2);
                                                                _.each(data.results, function(publication) {
                                                                    var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, camUserA, camUserB) {
                        ...
                        // Create a new publication
                        RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A', camUserA.user.id], null, null, function(err, data, result) {
                            assert.ok(!err);
                            assert.ok(data.publication);
                            createdPublications.push(data.publication);

                            // Create a new publication
                            RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'displayName B', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A', camUserA.user.id, camUserB.user.id], null, null, function(err, data, result) {
                                assert.ok(!err);
                                assert.ok(data.publication);
                                createdPublications.push(data.publication);

                                // Create a new publication
                                RestAPI.Publications.createPublication(camAdminRestContext, [generateRandomSourceId()], 'displayName C', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A', camUserB.user.id], null, null, function(err, data, result) {
                                    assert.ok(!err);
                                    assert.ok(data.publication);
                                    createdPublications.push(data.publication);

                                    // Request the publications for camUserA
                                    RestAPI.Publications.getPublications(camAdminRestContext, camUserA.user.id, null, 10, function(err, data) {
                                        assert.ok(!err);
                                        assert.equal(data.results.length, 2);
                                        _.each(data.results, function(publication) {
                                            var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
                                        ...
                                        RestAPI.Publications.getPublications(camAdminRestContext, camUserB.user.id, null, 10, function(err, data) {
                                            assert.ok(!err);
                                            assert.equal(data.results.length, 2);
                                            _.each(data.results, function(publication) {
                                                var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
                        ...
                        RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A'], null, null, function(err, data, result) {
                            ...
                            RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName B', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [camUser.user.id], null, null, function(err, data, result) {
                                ...
                                RestAPI.Publications.createPublication(camUser.restContext, [generateRandomSourceId()], 'displayName C', PublicationsConstants.publicationTypes.OTHER, 1388676959000, [camUser.user.id], null, null, function(err, data, result) {
                                    ...
                                    RestAPI.Publications.getPublicationsByCreator(anonymousRestContext, camUser.user.id, null, null, function(err, data) {
                                        assert.ok(!err);
                                        ...
                                        RestAPI.Publications.getPublicationsByCreator(gtAdminRestContext, camUser.user.id, null, null, function(err, data) {
                                            assert.ok(!err);
                                            assert.equal(data.results.length, 3);
                                            _.each(data.results, function(publication, i) {
                                                assert.ok(!publication.ticket);
                                            ...
                                            RestAPI.Publications.getPublicationsByCreator(halUser.restContext, camUser.user.id, null, null, function(err, data) {
                                                assert.ok(!err);
                                                assert.equal(data.results.length, 3);
                                                _.each(data.results, function(publication, i) {
                                                    assert.ok(!publication.ticket);
                                                ...
                                                RestAPI.Publications.getPublicationsByCreator(camUser.restContext, camUser.user.id, null, null, function(err, data) {
                                                    assert.ok(!err);
                                                    assert.equal(data.results.length, 3);
                                                    _.each(data.results, function(publication, i) {
                                                        var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
                                                    ...
                                                    RestAPI.Publications.getPublicationsByCreator(camAdminRestContext, camUser.user.id, null, null, function(err, data) {
                                                        assert.ok(!err);
                                                        assert.equal(data.results.length, 3);
                                                        _.each(data.results, function(publication) {
                                                            var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
                                                        ...
                                                        RestAPI.Admin.loginOnTenant(TestsUtil.createGlobalAdminRestContext(), 'localhost', function(err, ctx) {
                                                            assert.ok(!err);
                                                            RestAPI.Publications.getPublicationsByCreator(ctx, camUser.user.id, null, null, function(err, data) {
                                                                assert.ok(!err);
                                                                assert.equal(data.results.length, 3);
                                                                _.each(data.results, function(publication) {
                                                                    var createdPublication = _.find(createdPublications, function(createdPublication) { return createdPublication.id === publication.id; });
            ...
            ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-publications/publications/allowUserEntry': true}, function(err) {
                ...
                ConfigTestUtil.updateConfigAndWait(camAdminRestContext, null, {'oae-tickets/general/enabled': true}, function(err) {
                    ...
                    TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, coenegoUser, halUser) {
                        ...
                        // Create a new publication
                        RestAPI.Publications.createPublication(coenegoUser.restContext, [generateRandomSourceId()], 'displayName A', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author A'], null, null, function(err, data1, result) {
                            assert.ok(!err);
                            assert.ok(data1.publication);
                            createdPublications.push(data1.publication);

                            // Create a new publication
                            RestAPI.Publications.createPublication(halUser.restContext, [generateRandomSourceId()], 'displayName B', PublicationsConstants.publicationTypes.OTHER, 1388676959000, ['author B'], null, null, function(err, data2, result) {
                                assert.ok(!err);
                                assert.ok(data2.publication);
                                createdPublications.push(data2.publication);

                                // Requesting the publications created by the Cam user should be decorated with their corresponding tickets when using the tenant admin
                                RestAPI.Publications.getPublicationsByCreator(camAdminRestContext, coenegoUser.user.id, null, null, function(err, data1) {
                                    assert.ok(!err);
                                    ...
                                    // Requesting the publications created by the Cam user should be decorated with their corresponding tickets when using the tenant admin
                                    RestAPI.Publications.getPublicationsByCreator(camAdminRestContext, coenegoUser.user.id, null, null, function(err, data2) {
                                        assert.ok(!err);
