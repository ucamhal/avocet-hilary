...
var createPublication = module.exports.createPublication = function(tenantAlias, sourceIds, publication, callback) {
    ...
    var error;
    var validator = _getValidatorForCreatePublication(publication);
    if (validator.hasErrors()) {
...
var updatePublication = module.exports.updatePublication = function(ctx, publicationId, opts, callback) {
    ...
    // Validate the publication updates
    var validator = _getValidatorForUpdatePublication(publicationId, opts.publication);
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    }
...
/**
 * Returns a validator for a publication update
 *
 * @param  {String}           publicationId          The ID of the publication which needs to be updated
 * @param  {Object}           opts                   The object used to update the publication
 * @return {Validator}                               The validator object
 */
var _getValidatorForUpdatePublication = function(publicationId, publication) {
    var validator = new PublicationsValidator();
    validator.check(publicationId, {'code': 400, 'msg': 'Missing or invalid publicationId'}).isPublicationId();
    validator.check(null, {'code': 400, 'msg': 'Missing or invalid changes'}).isObject(publication);
    validator.check(_.keys(publication), {'code': 400, 'msg': 'Missing or invalid changes'}).min(1);

    if (publication) {
        if (!_.isUndefined(publication.embargoDate) && publication.embargoDate !== null) {
            validator.check(publication.embargoDate, {'code': 400, 'msg': 'Invalid embargoDate'}).isInt();
        }

        if (!_.isUndefined(publication.underEmbargo)) {
            validator.check(null, {'code': 400, 'msg': 'Invalid underEmbargo'}).isBoolean(publication.underEmbargo);
        }
    }
...
/**
 * Returns a validator for a new publication
 *
 * @param  {Publication}      publication            The publication to validate
 * @return {Validator}                               The validator object
 */
var _getValidatorForCreatePublication = function(publication) {
    var validator = new Validator();
    ...
    if (publication) {
        ...
        validator.check(publication.date, {'code': 400, 'msg': 'Missing or invalid date'}).isInt();
        ...
        if (publication.acceptanceDate !== null) {
            validator.check(publication.acceptanceDate, {'code': 400, 'msg': 'Invalid acceptanceDate'}).isInt();
        }
        ...
        if (publication.underEmbargo !== null) {
            validator.check(null, {'code': 400, 'msg': 'Invalid underEmbargo'}).isBoolean(publication.underEmbargo);
        }
    }
    return validator;
 ...
 * @param  {Publication[]}      publications            The publications which need to be augmented with the augmentations
 * @param  {Function}           callback                Standard callback function
 * @param  {Object}             callback.err            Error object containing the error message
 ...
 *
 * @param  {RestContext}        ctx                     The current execution context
 * @param  {Publication[]}      publications            The publications which need to be augmented with the content profiles
 * @param  {Function}           callback                Standard callback function
 * @param  {Object}             callback.err            Error object containing the error message
 * @param  {Publication[]}      callback.publications   The augmented publications
 * @api private
...
var _canManage = function(ctx, publication) {
    var contextUser = ctx.user();
    if (!contextUser) {
        return false;
    }

    // Check if the current user is a global admin
    if (contextUser.isGlobalAdmin()) {
        return true;
    }

    // Check if the current user is a tenant admin of the tenant the publication is uploaded in
    if (contextUser.isAdmin(publication.tenantAlias)) {
        return true;
    }
    ...
    return _.some(publication.authors, function(author) {
        return (author === contextUser.id || _isUserAdmin(contextUser, author));
    });
};
