 ...
 * @param  {Publication}    decorator.publication       The publication being decorated
        ...
        _augmentPublicationsWithUserProfiles(ctx, [publication], function(err, publications) {
            ...
            _augmentPublicationsWithContentProfiles(ctx, publications, function(err, publications) {
                ...
                var _finishDecorator = function() {
                    numDecorators--;
                    if (numDecorators === 0) {
                        return callback(null, _.extend(publication, decorations));
                    }
                };

                // When a publication is created, we want to run some decorators
                _.each(publicationsProfileDecorators, function(decorator, namespace) {
                    decorator(ctx, publicationId, function(err, decoration) {
                        if (err) {
                            log().error({'err': err, 'namespace': namespace}, 'Error when invoking a publication decorator');
                            return callback(err);
                        }
    ...
    PublicationsDAO.getPublicationsByCreator(userId, start, limit, function(err, publications, nextToken) {
        ...
        _augmentPublicationsWithUserProfiles(ctx, publications, function(err, publications) {
            ...
            _augmentPublicationsWithContentProfiles(ctx, publications, function(err, publications) {
                ...
                _augmentPublicationsWithTicketProfiles(ctx, publications, function(err, publications) {
                    if (err) {
                        return callback(err);
                    }
...
/**
 * Adds the ticket profile to each publication
 *
 * @param  {RestContext}        ctx                     The current execution context
 * @param  {Publication[]}      publications            A set of publications
 * @param  {Function}           callback                Standard callback method
 ...
 * @api private
 */
var _augmentPublicationsWithTicketProfiles = function(ctx, publications, callback) {

    // Filter the publication ID's from the publications collection
    var publicationIds = _.map(publications, function(publication) { return publication.id; });

    // Request all the corresponding tickets for the publications
    TicketsDAO.getTicketsByPublicationIds(publicationIds, function(err, tickets) {
        if (err) {
            log().warn({'code': err.code, 'msg': err.msg});
        }

        // Return the publications collection if no tickets were found
        if (!tickets) {
            return callback(null, publications);
        }

        // Attach the corresponding ticket to the publication
        _.each(publications, function(publication) {
            publication.ticket = _.find(tickets, function(ticket) { return publication.id === ticket.publicationId; });
        });
...
/**
 * Internal function that checks whether the current user is authorized to create a publication
 *
 * @param  {Context}        ctx                 Standard context object, representing the currently logged in user and its tenant
 * @return {Boolean}                            Whether the user is allowed to create a publication or not
 * @api private
 */
var _canEnterData = function(ctx) {
    ...
    // Users are allowed to create publications when the module is enabled for the tenant
    var enabled = PublicationsConfig.getValue(ctx.user().tenant.alias, 'publications', 'allowUserEntry');
    if (enabled) {
        return true;
    }

    // Admins should always be allowed to create publications, even if the module is disabled for the tenant
    if (!enabled && ctx.user().isAdmin(ctx.tenant().alias)) {
        return true;
    }
