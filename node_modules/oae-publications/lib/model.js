...
var Publication = module.exports.Publication = function(id, createdBy, linkedContentId, displayName, publicationType, date, thumbnailUri, authors, publisher, extra) {
    var that = {};
    extra = extra || {};

    // The OAE identifier for this publication
    that.id = id;

    // The id of the user that created the publication. (Can be `null' when using Symplectic)
    that.createdBy = createdBy || null;

    // The id of the linked content item
    that.linkedContentId = linkedContentId;

    // The title or display name for this publication
    that.displayName = displayName;

    // One of `PublicationConstants.publicationTypes`
    ...
    // Although the name is slightly ambiguous, this field is intended for Journal Names, Book publishers, ..
    that.publisher = publisher;

    // Extra options that depend on the publicationType
