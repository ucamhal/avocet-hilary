...
var addUserToRetryList = module.exports.addUserToRetryList = function(tenantAlias, user, callback) {
    ...
    var externalUserId = user.externalUserId || '';
    Cassandra.runQuery('INSERT INTO "SymplecticUsers" ("tenantAlias", "oaeUserId", "externalUserId") VALUES (?, ?, ?)', [tenantAlias, user.oaeUserId, externalUserId], callback);
};
...
var getUsersFromRetryList = module.exports.getUsersFromRetryList = function(tenantAlias, start, limit, callback) {
    Cassandra.runPagedQuery('SymplecticUsers', 'tenantAlias', tenantAlias, 'oaeUserId', start, limit, {'reversed': true}, function(err, rows, nextToken) {
        if (err) {
            return callback();
        }
        ...
        _.each(rows, function(row) {
            var oaeUserId = row.get('oaeUserId').value;
            var user = {
                'oaeUserId': oaeUserId
            };

            var externalUserId = row.get('externalUserId');
            if (externalUserId && externalUserId.value) {
                user.externalUserId = externalUserId.value;
            } else {
...
var deleteUsersFromRetryList = module.exports.deleteUsersFromRetryList = function(tenantAlias, users, callback) {
    ...
    _.each(users, function(user) {
        Cassandra.runQuery('DELETE FROM "SymplecticUsers" WHERE "tenantAlias" = ? AND "oaeUserId" = ?', [tenantAlias, user], callback);
    });
};
