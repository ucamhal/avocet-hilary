...
var refreshConfiguration = module.exports.refreshConfiguration = function(config, callback) {
    ...
    Cassandra.createColumnFamilies({
        'Symplectic': 'CREATE TABLE "Symplectic" ("tenantAlias" text PRIMARY KEY, "lastFullRun" timestamp, "lastIncrementalRun" timestamp)',
        'SymplecticUsers': 'CREATE TABLE "SymplecticUsers" ("tenantAlias" text, "oaeUserId" text, "externalUserId" text, PRIMARY KEY ("tenantAlias", "oaeUserId")) WITH COMPACT STORAGE'
    }, function(err) {
        if (err) {
            return callback(err);
        }
...
var ingestPublicationsForUser = function(tenantAlias, user, callback) {
    ...
    getPublicationsForUser(tenantAlias, user.externalUserId, function(err, publications) {
        ...
        ingestPublications(tenantAlias, publicationsToIngest, function(err, ingestedPublications) {
            if (err) {
                return callback(err);
            }
 ...
 * @api private
 */
var ingestPublications = function(tenantAlias, publications, callback, _ingestedPublications) {
    ...
    PublicationsAPI.IngesterAPI.ingestPublication(tenantAlias, ingesterPublication.publication, ingesterPublication.sourceIds, function(err, ingestedPublication, result) {
        if (err) {
            return callback(err);
        }

        _ingestedPublications.push(ingestedPublication);
        return ingestPublications(tenantAlias, publications, callback, _ingestedPublications);
    });
};
