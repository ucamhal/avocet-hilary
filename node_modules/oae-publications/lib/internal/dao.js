...
var createPublication = module.exports.createPublication = function(tenantAlias, publication, sourceIds, callback) {
    ...
    // 2. Link the publications to OAE users if they are an author
    var userIds = _.filter(publication.authors, AuthzUtil.isUserId);
    _generatePublicationsByUserQueries(queries, publication, userIds);

    // 3. Link they publications to OAE users if they created the publication
    if (publication.createdBy) {
        var hash = {};
        hash[publicationId] = 1;
        queries.push(Cassandra.constructUpsertCQL('PublicationsByCreator', 'userId', publication.createdBy, hash));
    }

    // 4. Add all the sources where this publication was found in
    ...
    // 5. Add all the authors for this publication
    var authors = publication.authors.slice(0);
    _createPublicationBatch(authors, function(err, authorRows) {
        if (err) {
            return callback(err);
        }

        // When all the authors have been retrieved, create a new query
        queries.push(Cassandra.constructUpsertCQL('PublicationsAuthors', 'publicationId', publicationId, authorRows));
 ...
 * Creates a batch
 *
 * @param  {String[]}   authors             Collection of authors
 * @param  {Object}     authorRows          Object that contains all the authors
 * @param  {Function}   callback            Standard callback function
 * @param  {Object}     callback.err        Standard error object (if any)
 */
var _createPublicationBatch = function(authors, callback) {
    if (_.isEmpty(authors)) {
        return callback({'code': 400, 'msg': 'A publication should at least have one author'});
    }

    var authorRows = {};
    var userIds = _.filter(authors, AuthzUtil.isUserId);

    if (userIds.length) {

        // Filter the non-OAE users from the authors collection
        var notUserIds = _.filter(authors, function(author) { return !AuthzUtil.isUserId(author); });

        // Get the user profile from the OAE-users
        PrincipalsDAO.getPrincipals(userIds, null, function(err, principals) {
            if (err) {
                return callback(err);
            }

            // Add the OAE-users to the authors collection
            _.each(principals, function(principal) {
                authorRows[principal.id] = principal.displayName;
            });

            // Add the non-OAE users back to the authors collection
            _.each(notUserIds, function(user) {
                authorRows[user] = user;
            });

            return callback(null, authorRows);
        });

    } else {
        _.each(authors, function(author) {
            authorRows[author] = author;
        });
...
var updatePublication = module.exports.updatePublication = function(publicationId, fields, callback) {
    ...
    // Construct the query
    var q = Cassandra.constructUpsertCQL('Publications', 'publicationId', publicationId, fields, 'QUORUM');
    if (!q) {
        return callback({'code': 500, 'msg': 'Unable to store publication fields'});
    }

    // Send the query off to Cassandra
    Cassandra.runQuery(q.query, q.parameters, function(err) {
        ...
        getPublication(publicationId, function(err, publication) {
            if (err) {
                return callback(err);
            }
 ...
 * @param  {Publication}   publication     The publication that needs to be linked to a user
 * @param  {String[]}      userIDs         A collection of userIds
 */
var _generatePublicationsByUserQueries = function(queries, publication, userIds) {
...
var linkPublicationToUser = module.exports.linkPublicationToUser = function(publication, authorName, userId, callback) {
    ...
    _generatePublicationsByUserQueries(queries, publication, [userId]);
...
var getPublicationsByCreator = module.exports.getPublicationsByCreator = function(userId, start, limit, callback) {
    ...
    // Publications are typically ingested on an interval, so we can relax the consistency requirement
    Cassandra.runPagedColumnQuery('PublicationsByCreator', 'userId', userId, start, limit, {'reversed': true, 'consistency': 'ONE'}, function(err, columns, nextToken) {
        if (err) {
            return callback(err);
        }
...
var _rowToPublication = function(row) {
    ...
    return new Publication(hash.publicationId, hash.createdBy, hash.linkedContentId, hash.displayName, hash.publicationType, hash.date, hash.thumbnailUri, [], hash.publisher, hash);
};
