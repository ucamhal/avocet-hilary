...
var updatePublication = module.exports.updatePublication = function(publicationId, fields, callback) {
    ...
    _clearAcceptanceDateIfNull(publicationId, fields.acceptanceDate, function(err) {
        if (err) {
            return callback(err);
        }
...
/**
 * Clear the acceptanceDate for a specific publication if the supplied value is null
 *
 * @param  {String}         publicationId           The ID of the publication the authors need to be cleared for
 * @param  {Number}         acceptanceDate          The acceptance date the publication needs to be updated with
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            Standard error object, if any
 * @api private
 */
var _clearAcceptanceDateIfNull = function(publicationId, acceptanceDate, callback) {
    if (!_.isUndefined(acceptanceDate) && acceptanceDate === null) {
        return _clearAcceptanceDateForPublication(publicationId, callback);
    }
    callback(null);
};

/**
 * Clear the acceptanceDate for a specific publication
 *
 * @param  {String}         publicationId           The ID of the publication the authors need to be cleared for
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            Standard error object, if any
 * @api private
 */
var _clearAcceptanceDateForPublication = function(publicationId, callback) {
    Cassandra.runQuery('DELETE "acceptanceDate" FROM "Publications" WHERE "publicationId" = ?', [publicationId], function(err) {
