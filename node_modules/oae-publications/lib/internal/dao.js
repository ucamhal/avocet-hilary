...
var createPublication = module.exports.createPublication = function(tenantAlias, publication, sourceIds, callback) {
    ...
    _createPublicationBatch(authors, function(err, authorRows) {
        ...
        // When all the authors have been retrieved, create a new quey
        _.each(authorRows, function(val, key) {
            queries.push({'query': 'INSERT INTO "PublicationsAuthors" ("publicationId", "author", "value") VALUES (?, ?, ?)', 'parameters': [publicationId, key, val]});
        });
...
var getPublicationsForUser = module.exports.getPublicationsForUser = function(userId, start, limit, callback) {
    ...
    Cassandra.runPagedQuery('PublicationsByUser', 'userId', userId, 'publicationId', start, limit, {'reversed': true, 'consistency': 'ONE'}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }
...
var getPublicationsByCreator = module.exports.getPublicationsByCreator = function(userId, start, limit, callback) {
    ...
    // Publications are typically ingested on an interval, so we can relax the consistency requirement
    Cassandra.runPagedQuery('PublicationsByCreator', 'userId', userId, 'publicationId', start, limit, {'reversed': true, 'consistency': 'ONE'}, function(err, rows, nextToken) {
        if (err) {
            return callback(err);
        }
