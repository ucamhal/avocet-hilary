...
var createPublication = module.exports.createPublication = function(publication, sourceIds, callback) {
    ...
    _createPublicationBatch(authors, null, function(err, authorRows) {
        if (err) {
            return callback(err);
        }
 ...
 * @param  {Object}     callback.err        Standard error object (if any)
 */
var _createPublicationBatch = function(authors, authorRows, callback) {
    ...
    // Check if the author is a user id
    if (AuthzUtil.isUserId(author)) {

        // Get the author's user id
        PrincipalsDAO.getPrincipal(author, function(err, principal) {
            if (err) {
                return callback(err);
            }

            // Fetch the user's display name
            authorRows[author] = principal.displayName;
            return _createPublicationBatch(authors, authorRows, callback);
        });

    // Add the original string if the author is not an OAE user
    } else {
