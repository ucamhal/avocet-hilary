...
describe('Publications - Symplectic', function() {
    ...
    it('verify basic ingesting', function(callback) {
        ...
        SymplecticIngester.startIngesting(function() {
            ...
            PublicationsTestUtil.getAllPublicationsForUser(users.alanblackwell.restContext, users.alanblackwell.user.id, function(err, allPublications) {
                ...
                assert.strictEqual(allPublications.length, 91);
                ...
                assert.strictEqual(dasherPublication.publicationType, 'journal article');
                assert.strictEqual(dasherPublication.publisher, 'HUM-COMPUT INTERACT');
                assert.strictEqual(dasherPublication.authors.length, 3);
                ...
                PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 25, function(err, data) {
                    assert.ok(!err);
                    assert.strictEqual(data.results.length, 1);

                    var publication = data.results[0];
                    assert.strictEqual(publication.displayName, 'Practitioner-customizable clinical information systems: a case study to ground further research and development opportunities');
                    assert.strictEqual(publication.publicationType, 'journal article');
                    assert.strictEqual(publication.publisher, 'Journal of Healthcare Engineering');
    ...
    it('verify a publication can be linked to multiple users', function(callback) {
        ...
        SymplecticIngester.ingestPublicationsForTenant(global.oaeTests.tenants.cam.alias, true, null, function(err) {
            ...
            PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 25, function(err, data) {
                ...
                assert.strictEqual(publication.authors.length, 3);
    ...
    it('verify request retry logic', function(callback) {
        ...
        SymplecticIngester.ingestPublicationsForTenant(global.oaeTests.tenants.cam.alias, true, null, function(err) {
            ...
            assert.strictEqual(failsSent, 2);
            ...
            PublicationsTestUtil.getAllPublicationsForUser(users.alanblackwell.restContext, users.alanblackwell.user.id, function(err, publications) {
                ...
                PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 10, function(err, data) {
                    assert.ok(!err);
                    assert.ok(publications.length > 0);
                    PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 10, function(err, data) {
                        assert.ok(!err);
    ...
    it('verify user retry logic', function(callback) {
        ...
        SymplecticIngester.startIngesting(function() {
            ...
            PublicationsRestAPI.getPublicationsByAuthor(users.alanblackwell.restContext, users.alanblackwell.user.id, null, 10, function(err, data) {
                assert.ok(!err);
                ...
                PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 10, function(err, data) {
                    ...
                    PublicationsRestAPI.getPublicationsByAuthor(users.cecilymorrison.restContext, users.cecilymorrison.user.id, null, 10, function(err, data) {
                        ...
                        SymplecticDAO.getUsersFromRetryList(global.oaeTests.tenants.cam.alias, null, 10, function(err, retryUsers) {
                            assert.ok(!err);
                            assert.strictEqual(retryUsers.length, 1);
                            assert.strictEqual(retryUsers[0].oaeUserId, users.alanblackwell.user.id);
                            assert.strictEqual(retryUsers[0].externalUserId, 'afb21');
                            ...
                            SymplecticIngester.startIngesting(function() {
                                ...
                                SymplecticDAO.getLastRunTimes(global.oaeTests.tenants.cam.alias, function(err, lastFullRun, lastIncrementalRun) {
                                    ...
                                    PublicationsRestAPI.getPublicationsByAuthor(users.alanblackwell.restContext, users.alanblackwell.user.id, null, 10, function(err, data) {
                                        ...
                                        SymplecticDAO.getUsersFromRetryList(global.oaeTests.tenants.cam.alias, null, 10, function(err, retryUsers) {
                                            assert.ok(!err);
                                            assert.strictEqual(retryUsers.length, 0);
                ...
                AuthenticationDAO.associateLoginId(new LoginId(global.oaeTests.tenants.cam.alias, AuthenticationConstants.providers.CAS, 'sg555'), simong.user.id, function(err) {
                    ...
                    SymplecticIngester.startIngesting(function() {
                        ...
                        PublicationsTestUtil.getAllPublicationsForUser(simong.restContext, simong.user.id, function(err, publications) {
                            ...
                            assert.strictEqual(publications[0].displayName, 'Messing About');
                            assert.strictEqual(publications[0].publicationType, 'journal article');
                            assert.strictEqual(publications[0].publisher, 'Jiggery and Pokery Press');
                            assert.strictEqual(publications[0].authors.length, 2);
                            assert.strictEqual(_.find(publications[0].authors, function(author) { return (typeof author === 'string'); }), 'Visser B');
                            assert.strictEqual(_.find(publications[0].authors, function(author) { return (typeof author === 'object'); }).id, simong.user.id);
